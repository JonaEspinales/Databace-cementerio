...
select FECHAT,NOMBRESE,APELLIDOSE,TIPOTRAMITE,HORAT,CANTIDADT,VALORT 
from TRAMITE inner join EMPLEADO 
on TRAMITE.ID_EMPLEADO= EMPLEADO.ID_EMPLEADO
where TRAMITE.TIPOTRAMITE='CERTIFICACION'
group by NOMBRESE,APELLIDOSE,TIPOTRAMITE,FECHAT,HORAT,CANTIDADT,VALORT;
...
select (TIPOCONSTRUCCION)as TIPO_ESTRUCTURA ,UBICACION,ALTURA,NUMERO from CONSTRUCCIONP
where CONSTRUCCIONP.TIPOCONSTRUCCION='BOBEDAS' OR CONSTRUCCIONP.TIPOCONSTRUCCION='NICHOS' 
      OR CONSTRUCCIONP.TIPOCONSTRUCCION='FOSA CUMUN'
group by TIPOCONSTRUCCION,UBICACION,ALTURA,NUMERO
order by UBICACION;
